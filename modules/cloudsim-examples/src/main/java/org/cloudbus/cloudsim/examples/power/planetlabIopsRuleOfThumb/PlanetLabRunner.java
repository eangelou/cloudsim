...
import org.cloudbus.cloudsim.examples.power.HelperIopsRuleOfThumb;
...
public class PlanetLabRunner extends RunnerAbstract {
	...
	protected void init(String inputFolder) {
		try {
			...
			broker = HelperIopsRuleOfThumb.createBroker();
			int brokerId = broker.getId();

			cloudletList = PlanetLabHelper.createCloudletListPlanetLab(brokerId, inputFolder);
			vmList = HelperIopsRuleOfThumb.createVmList(brokerId, cloudletList.size());
			hostList = HelperIopsRuleOfThumb.createHostList(PlanetLabConstants.NUMBER_OF_HOSTS);
		} catch (Exception e) {
			e.printStackTrace();
